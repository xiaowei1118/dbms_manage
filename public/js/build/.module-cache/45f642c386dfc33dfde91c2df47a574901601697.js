 ...
 * @param  {[type]} loadCommentsFromServer:function(){                                                                                                                                            $.ajax({                   url:this.pros.url1,                                              data:{} [description]
 ...
 * @param  {String} changeTable2DataFromServer:function(id){                 $.ajax({                                                   url:this.url2+"&"+this.pros.url2_link_id+" [description]
 * @param  {[type]} dataType:"json"                           [description]
...
var LinkedDataTable = React.createClass({displayName: "LinkedDataTable",
  ...
  loadCommentsFromServer:function(){        //获取服务端数据
    console.log(this.pros.data);
    $.ajax({
      url:this.pros.data.url1,
      data:{},
      dataType:"json",
  ...
  changeTable2DataFromServer:function(id){
    $.ajax({
      url:this.pros.data.url2+"&"+this.pros.data.url2_link_id+"="+id,         //获取联动表格数据
      data:{},
      dataType:"json",
      success:function(data){
  ...
  table1rowclick:function(data){
      var id=this.pros.data.url1_link_id;
